...
def warmup(a2c, pool, n_iter, step_size, n_steps, config):
    for _ in range(n_iter):
        obs_seq, _, idxs, rewards, _, _, _, m_logits, m_values = \
            pool.master_interact(step_size, n_steps)
        loss, grad_norm, entropy, values, al, cl = a2c.step(
            idxs,
            rewards.sum(-1),
            np.ones((config["n_env"], n_steps)),
            m_logits,
            m_values
        )


def joint_train(m_a2c, s_a2c, pool, n_iter, step_size, n_steps, n_env):
    rewards_per_epoch = 0
